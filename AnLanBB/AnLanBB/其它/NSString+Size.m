...
+(NSString *)stringWithTimelong:(int)timeLong{
    NSInteger  h =timeLong/3600;
    NSInteger m = (timeLong%3600)/60;
    NSInteger s = timeLong%3600%60;
    
    if (h>0) {
       return [NSString stringWithFormat:@"%ld:%.2ld:%.2ld",h,(long)m,(long)s];
    }else {
        return [NSString stringWithFormat:@"%.2ld:%.2ld",(long)m,(long)s];
    }
}

+(NSString *)stringWithVideoTime:(NSString *)video_time
{
    NSInteger time = [video_time intValue];
    
    NSInteger  h =time/3600;
    NSInteger m = (time%3600)/60;
    NSInteger s = time%3600%60;
...
+ (NSString *)stringHHMMSSWithTime:(CGFloat)time{
    NSString * str = @"";
    NSInteger  h = (NSInteger)time/3600;
    NSInteger m = ((NSInteger)time%3600)/60;
    NSInteger s = (NSInteger)time%3600%60;
    return [NSString stringWithFormat:@"%ld:%.2ld:%.2ld",(long)h,(long)m,(long)s];
}
...
+ (NSUInteger)durationWithVideo:(NSString  *)videoUrl{
    ...
    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:[NSURL URLWithString:videoUrl] options:opts]; // 初始化视频媒体文件
    NSUInteger second = 0;
