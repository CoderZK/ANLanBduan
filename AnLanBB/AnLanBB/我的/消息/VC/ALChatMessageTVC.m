...
- (void)getData {
    ...
    [zkRequestTool networkingPOST:[QYZJURLDefineTool appB_findUserLogsListURL] parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)sentMessageWith:(NSString *)str withType:(NSString *)type {
       ...
       dict[@"type"] = type;
       dict[@"content"] = str;
       [zkRequestTool networkingPOST:[QYZJURLDefineTool user_appB_sendChatMessageURL] parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
           ...
           if ([responseObject[@"key"] intValue]== 1) {
               ...
               if ([type isEqualToString:@"1"]) {
                   ...
                   model.toName = self.nameStr;
                   model.toUserId = self.toUserId;
                   model.toAvatar = self.toAvatar;
