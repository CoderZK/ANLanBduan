...
- (void)getDataWithStarTime:(NSDate *)starData  isAllMonth:(BOOL)isAllMonth{
    ...
    [zkRequestTool networkingPOST:[QYZJURLDefineTool user_appB_findCalendarListURL] parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            if (isAllMonth) {
                ...
                for (ALMessageModel *  model  in  [ALMessageModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"]]) {
                    ...
                    if (model.scheduleDate.length >= 16) {
                        model.time = [model.scheduleDate substringWithRange:NSMakeRange(11, 5)];
                    }
                    if (model.isSend) {
...
- (void)keyboardDidHideShow:(NSNotification *)notification {
    ...
    if (sstatusHeight > 20) {
        self.whiteView.mj_y = ScreenH - 100 - 49 - 34 ;
    }else {
        self.whiteView.mj_y = ScreenH - 100 - 49 ;
    }
    self.wwView.hidden = YES;
    self.faBuBt.hidden = NO;
...
- (void)addCalaer {
    ...
    NSMutableDictionary * dict = @{}.mutableCopy;
    NSDateFormatter * formatterTwo = [[NSDateFormatter alloc] init];
    [formatterTwo setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    formatterTwo.timeZone = [NSTimeZone timeZoneWithAbbreviation:@"GMT+0800"];
    NSString * ymd = [formatterTwo stringFromDate:[NSDate date]];
    dict[@"scheduleTime"] =  ymd;
    if (self.timeStr.length > 0) {
        dict[@"scheduleTime"] = self.timeStr;
    }
    dict[@"content"] = self.TF.text;
    [zkRequestTool networkingPOST:[QYZJURLDefineTool user_appB_addUserCalendarURL] parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)didClickALCJianKangTiXingHomeCell:(ALCJianKangTiXingHomeCell *)cell withIndex:(NSInteger)index {
    ...
    if (indexPath.row == 0) {
        ...
        if (index >= 10000) {
            ...
            vc.sendSuccessBlock = ^{
                for (int i = 0 ; i < weakSelf.dataArray.count; i++) {
                    if ([weakSelf.dataArray[i].calendarId isEqualToString:self.isNOFinshArr[index - 10000].calendarId]) {
                        weakSelf.dataArray[i].isSend = YES;
                        break;
                    }
                }
                [weakSelf.tableView reloadData];
            };
            vc.calendarId = self.isNOFinshArr[index - 10000].calendarId;
            ...
            vc.hidesBottomBarWhenPushed = YES;
            Weak(weakSelf);
            vc.isSendBlock = ^(NSString * _Nonnull ID) {
                for (int i = 0 ; i < weakSelf.dataArray.count; i++) {
                    if ([weakSelf.dataArray[i].calendarId isEqualToString:self.isNOFinshArr[index].ID]) {
                        weakSelf.dataArray[i].isSend = YES;
                        break;
                    }
                }
        ...
        if (index >= 10000) {
            ...
            ALCTiXingDetailTVC * vc =[[ALCTiXingDetailTVC alloc] initWithTableViewStyle:(UITableViewStyleGrouped)];
            vc.hidesBottomBarWhenPushed = YES;
            vc.dataModel = self.isFinshArr[index - 10000];
            [self.navigationController pushViewController:vc animated:YES];
